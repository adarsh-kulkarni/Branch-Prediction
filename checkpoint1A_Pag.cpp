...
bool BTB_prediction(ADDRINT ins_ptr)
{
    ...
    predict = BTB_Table[(BTB[index].HistoryReg & 0xFFF)]; 
...
VOID BTB_update(ADDRINT ins_ptr, bool taken)
{
    ...
    int counter;
    counter =  BTB_Table[BTB[index].HistoryReg & 0xFFF];  
    ...
    if(taken){
     ...
     BTB_Table[BTB[index].HistoryReg & 0xFFF] = counter;
...
VOID BTB_insert(ADDRINT ins_ptr)
{
   ...
   // BTB_update(ins_ptr,true);
    ...
    BTB_Table[BTB[index].HistoryReg & 0xFFF] = 2;
...
VOID WriteResults(bool limit_reached)
{
    ...
    out << "Misses in the BTB " << BTBMiss << endl;
    out << "Hits in BTB " << BTBHit << endl;
